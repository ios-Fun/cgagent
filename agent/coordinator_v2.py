...
# from .errors import AgentError, ExecutorError, PlannerError
...
#
#     Coordinates the entire pipeline using LLM for both planning and routing:
#     1. Initialize context with available skills
#     2. LLM Planner analyzes intent and selects skills (No Rasa)
#     3. Executor executes skills with progressive disclosure
#     4. Synthesizer generates final response
#
#     Fallback: If no skills are matched by LLM, falls back to general conversation.
#     """
#
...
#
#             # Phase 1: Planning (LLM-based routing)
#             plan = await self._plan_execution(user_input)
#
#             # Phase 2: Execute skills
#             execution_result = self._execute_plan(plan)
#
#             # Check for critical execution failures
#             # Note: For general chat (empty steps), success is True but executed_steps is empty
#             if not execution_result["success"]:
#                  # If it failed and it wasn't just a general chat fallback
#                  if plan.steps:
#                     return {
...
#
#         This method replaces Rasa. It uses the LLM Planner to:
#         1. Analyze user intent.
#         2. Match against available skills.
#         3. Return a plan. If no skills match, returns a general conversation plan.
#
#         Args:
#             user_input: User's request
#
...
#
#         # Get all available skills for the LLM Planner to choose from
...
#
#         try:
#             # Attempt standard planning via LLM
#             # The Planner will analyze the prompt and decide which skills to use
#             plan = self.planner.generate_plan(user_input, available_skills, conversation_history)
#
#             # Check if the plan actually has steps.
#             # The planner filters by confidence, so if steps are empty, LLM found no suitable skill.
#             if not plan.steps:
#                 is_general_chat = True
#                 logger.info("LLM Planner determined no specific skills are needed. Switching to general conversation mode.")
#
#         except PlannerError as e:
#             # If planner fails explicitly (e.g., JSON parse error, or internal logic failure)
#             logger.warning(f"Planner failed to generate a valid plan: {e}. Switching to general conversation mode.")
#             is_general_chat = True
#         except Exception as e:
#             logger.error(f"Unexpected error during LLM planning: {e}. Switching to general conversation mode.")
#             is_general_chat = True
#
#         if is_general_chat:
#             # Create a fallback plan for general conversation
#             # This plan has an intent but NO steps.
#             # The Executor will skip execution, and Synthesizer will handle pure LLM chat.
#             plan = ExecutionPlan(
#                 intent="general_conversation",
#                 steps=[], # Empty steps indicate no specific tool/skill usage
#                 estimated_tokens=0
#             )
#             logger.info("Created general conversation fallback plan.")
#
...
#
#         # If there are no steps (General Chat Mode), return success immediately
#         # so the synthesizer can take over with just the user input and history.
#         if not plan.steps:
#             logger.info("Skipping execution phase for general conversation plan.")
#             return {
#                 "success": True,
#                 "executed_steps": [],
#                 "failed_steps": [],
